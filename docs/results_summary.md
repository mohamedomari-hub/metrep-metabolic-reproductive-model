...
**Figure 1.** AUC-based one-at-a-time local sensitivity ranking. Blue
indicates positive AUC response and orange indicates negative AUC response.
...
**Figure 3.** Strongest PRCC associations between parameters and biomarker
AUCs.
...
**Figure 4.** PRCC associations for the representative 3 x 3
...
![Sensitivity-compensation decision regions](../results_final/figures/identifiability_parameter_scenario_map.png)

**Figure 7.** Sensitivity-versus-compensation decision map from the SVD
identifiability screen. Dashed thresholds separate parameters by output
sensitivity and weak-direction participation, supporting practical
classification into estimation candidates, fixed-anchor parameters, and
low-information parameters.

Figure 7 translates the SVD screen into practical calibration guidance.
Parameters with high sensitivity and low weak-direction participation are more
suitable for estimation, whereas sensitive parameters with high compensation
involvement may be biologically influential but difficult to estimate jointly.
Parameters with low sensitivity under the current observable panel provide
limited information for calibration in this setting.
...
**Figure 12.** Parameter-specific biomarker information ranking.

Figure 12 ranks observable biomarkers by information content for the
...
**Figure 13.** Guided prior-to-posterior updates using parameter-specific
biomarker-day observations.

Figure 13 shows that practically identifiable parameters generally undergo
...
**Figure 14.** Posterior comparison between high- and low-information sampling
days.

Figure 14 checks whether MI-ranked days produce more interpretable posterior
...
**Figure 15.** Reduced ODE-archive posterior using likelihood weights over
precomputed broad-prior simulations.

The reduced ODE-archive posterior in Figure 15 uses only precomputed ODE rows.
...
**Figure 16.** Archive-based sequential ABC posterior summaries for
parameter-specific guided scenarios.

Figure 16 shows archive-based sequential ABC filtering. The tolerance is
